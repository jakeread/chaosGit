...
float[][] dist = new float[22][61]; //always at k, i (servo, dyna angles -> *5);
float[][] temp = new float[22][61];
...
void draw()
{
  ...
  for (int k = 0; k<dist.length; k++) {
    for (int i = 0; i<dist[0].length; i++) {
      ...
      x = sin(radians(i*5-150))*cos(radians(-k*6+90));
      y = sin(radians(i*5-150))*sin(radians(-k*6+90));
      z = cos(radians(i*5-150));
  ...
  if ( myPort.available() > 0) {  // If data is available,
    ...
    if (inBuffer != null) {
      ...
      if (arrBuffer.length != 4) {
        ...
        kIndex = int(arrBuffer[0])/5;
        iIndex = int(arrBuffer[1])/5;
        if (kIndex <= 22 && iIndex <= 61) {
